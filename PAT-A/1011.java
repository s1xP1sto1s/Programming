...
Each input file contains one test case. Each case contains the betting information of 3 games. Each game occupies a line with 
...
class Main
{
  public static void main(String[] args)
  {
    ...
    for(int i=0;i<3;i++)
    {
      ...
      int index = find(str);
      profile *= Double.parseDouble(str[index]);
      switch(index)
      {
        case 0:
        {
    ...
    profile = (double)Math.round((profile*0.65-1)*2*100)/100;
    sb.append(" "+profile);
  ...
  public static int find(String[] str)
  {
    ...
    {
      if(Double.parseDouble(str[i])>Double.parseDouble(str[max]))
        max = i;
    }
    return max;
  }
